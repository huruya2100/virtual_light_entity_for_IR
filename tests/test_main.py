...
from virtual_light_entity_for_IR.main import Config, MQTTClient, Light, HomeAssistantClient, BaseMQTTClient, MQTTPublisher
import json
...
def test_light_change_virtual_state_brightness_increase():
    ...
    with patch.object(light.home_assistant, 'call_script_service') as mock_call, \
         patch.object(light.mqtt, 'connect'), \
         patch.object(light.mqtt, 'publish'), \
         patch.object(light.mqtt, 'disconnect'), \
         patch('time.sleep'):
...
def test_light_change_virtual_state():
    ...
    with patch.object(light, 'change_virtual_state_state') as mock_state, \
         patch.object(light, 'change_virtual_state_brightness') as mock_brightness:
