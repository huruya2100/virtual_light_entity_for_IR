...
class HomeAssistantClient:
        ...
        try:
            for i in range(repeat):
                ...
                if response.status_code != 200:
                    logger.warning(f"APIコール失敗: {response.status_code} {response.text}")
                time.sleep(0.5)
            ...
            logger.error(f"Home Assistant APIの呼び出しに失敗しました: {e}", exc_info=True)
            return 0, str(e)
    ...
    def get_entity_state(self, entity_id: str) -> Tuple[int, Dict[str, Any]]:
        ...
        try:
            ...
            if status_code == 200:
                ...
                logger.warning(f"エンティティ状態取得失敗: {status_code} {response.text}")
                return status_code, {}
        except Exception as e:
            logger.error(f"エンティティ状態取得中にエラーが発生しました: {e}", exc_info=True)
            return 0, {}
    
    def call_service(self, domain: str, service: str, service_data: Dict[str, Any]) -> Tuple[int, str]:
        """
        ...
        try:
            ...
            logger.error(f"サービス呼び出し中にエラーが発生しました: {e}", exc_info=True)
            return 0, str(e)
    ...
    def get_device_info(self, device_id: str) -> Tuple[int, Dict[str, Any]]:
        ...
        try:
            ...
            logger.error(f"デバイス情報取得中にエラーが発生しました: {e}", exc_info=True)
            return 0, {}